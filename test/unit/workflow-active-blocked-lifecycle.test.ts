...
function startRecoverableRun(store: ReturnType<typeof createStore>, runId: string, parentRunId?: string): number {
	...
	assert.equal(
		store.recordRunBlocked(runId, "Configure credentials and resume.", {
			failureKind: "auth",
...
function install(store: ReturnType<typeof createStore>, state = createWorkflowLifecycleNotificationState()) {
...
describe("active recoverable blocked lifecycle notices", () => {
	...
	test("re-notifies when the same runId blocks again at a new blockedAt occurrence", () => {
		...
		assert.equal(
			store.recordRunBlocked("reused-id", "Configure credentials and resume.", {
				failureKind: "auth",
				failureCode: "missing_api_key",
				failureRecoverability: "recoverable",
				failureDisposition: "active_blocked",
	...
	test("does not re-send a still-pending admission across a config reinstall", async () => {
		...
		const reinstalled = installWorkflowLifecycleNotifications({
			...
			config,
			sendMessage(message) {
				newSends.push(message as SentMessage);
				return Promise.resolve();
			},
		});
