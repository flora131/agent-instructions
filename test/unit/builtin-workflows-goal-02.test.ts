...
describe("goal", () => {
	...
	test("forks later orchestrator turns from the prior orchestrator session without forking reviewers", async () => {
		...
		assert.equal(ctx.calls.taskOptions["orchestrator-2"]?.[0]?.forkFromSessionFile, "/tmp/goal-orchestrator-1.jsonl");
		assert.match(
		...
		for (const reviewerName of ["completion-reviewer-2", "evidence-reviewer-2", "risk-reviewer-2"]) {
			assert.equal(ctx.calls.taskOptions[reviewerName]?.[0]?.context, undefined, reviewerName);
			assert.equal(ctx.calls.taskOptions[reviewerName]?.[0]?.forkFromSessionFile, undefined, reviewerName);
		}
	});
	...
	test("passes only latest reviewer artifacts into later orchestrator continuation", async () => {
			...
			{ objective: "Finish the migration" },
			{
				task: (name, _options, calls) => {
					if (
						...
						const reviewingFinalTurn = calls.task.includes("orchestrator-3");
		...
		assert.match(thirdTurnPrompt, /Latest available review artifacts/);
		const thirdTurnReads = readPaths(ctx.calls.taskOptions["orchestrator-3"]?.[0]);
		assert.equal(
			thirdTurnReads.filter((path) => /review-[a-z-]+-reviewer\.json$/.test(normalizePathSeparators(path))).length,
			3,
		);
	});
	...
	test("uses schema-backed reviewer stages without prompt tool nudges", async () => {
		...
		assert.match(ctx.calls.prompts["completion-reviewer-1"]?.[0] ?? "", /echo the prior blocker string/i);
		for (const reviewerName of ["completion-reviewer-1", "evidence-reviewer-1", "risk-reviewer-1"]) {
