...
function seedDurableOnly(backend: InMemoryDurableBackend, workflowId: string, withProgress = true): void {
	backend.registerWorkflow({
		workflowId,
		name: "restored-workflow",
		inputs: {},
...
describe("gracefully quit durable workflow session restore", () => {
	...
	test.sequential.each([
		...
		assert.equal(
			reconcileDurableResumeShadow(run, localStore, {
				backend,
				jobs,
	...
	test.sequential.each(["paused", "running"] as const)(
		...
		async (durableStatus) => {
			...
			assert.match(result.message, /not resumable|no durable progress/i);
			assert.deepEqual(
				store.runs().find((run) => run.id === workflowId),
				before,
			);
