...
`,
		...
		writeFileSync(
			join(fixtureDir, "startup-warmup.ts"),
			`export function scheduleMcpStartupWarmup() { return { cancel() {} }; }\n`,
		);
		writeFileSync(
			join(fixtureDir, "mcp-auth-flow.ts"),
			`
let resets = 0;
...
`,
		...
		writeFileSync(
			join(fixtureDir, "proxy-modes.ts"),
			`
