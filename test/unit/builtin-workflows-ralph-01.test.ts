...
describe("ralph", () => {
	...
	const approvingRalphReview = JSON.stringify({
		...
		requirements_traceability: [
			{ requirement: "complete requested task", status: "proven", evidence: "current state proves the task" },
		],
		stop_review_loop: true,
		reviewer_error: null,
	});

	const approveReviewers = (name: string): string | undefined =>
	...
	test("starts Ralph with raw-prompt research refinement and research prompts", async () => {
		...
		assert.equal(researchPrompt.trimEnd().endsWith("</instruction>"), true);
		...
		assert.deepEqual(ctx.calls.taskOptions["research-prompt-refinement-1"]?.[0]?.excludedTools, [
			"ask_user_question",
		]);
		...
		const ctxWithCriteria = makeMockCtx({
			prompt: "Follow-up delta",
			acceptance_criteria: "Original task contract",
			max_loops: 1,
			base_branch: "main",
			git_worktree_dir: "",
			create_pr: false,
		...
		const criteriaPrompts = [
			ctxWithCriteria.calls.prompts["research-prompt-refinement-1"]?.[0] ?? "",
			ctxWithCriteria.calls.prompts["research-1"]?.[0] ?? "",
			ctxWithCriteria.calls.prompts["orchestrator-1"]?.[0] ?? "",
			ctxWithCriteria.calls.prompts["reviewer-a"]?.[0] ?? "",
		];
	...
	test("adds workflow cwd context to every Ralph stage prompt", async () => {
		...
		for (const [label, prompt] of prompts) {
			...
			assert.match(prompt, /Shell commands and relative file paths should be relative to this directory/i, label);
	...
	test("does not add final handoff language to earlier stages when create_pr is false", async () => {
		...
		assert.match(orchestratorPrompt, /Delegate only work that is genuinely independent/);
	});
