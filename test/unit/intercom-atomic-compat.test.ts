	...
	withEnv(env, () => {
		const api = {
			...
			registerTool: ((tool: ToolDefinition) => {
				...
				captured.toolNames.push(tool.name);
...
describe("intercom package manifest compatibility", () => {
	...
	test("loader prefers atomic package metadata and still accepts legacy pi metadata", async () => {
		...
		writeFileSync(
			join(atomicPackageDir, "atomic.ts"),
			"export default (pi) => pi.registerCommand('from-atomic', { description: '', handler() {} });\n",
			"utf8",
		);
		writeFileSync(
			join(atomicPackageDir, "pi.ts"),
			"export default (pi) => pi.registerCommand('from-pi', { description: '', handler() {} });\n",
			"utf8",
		);
		...
		writeFileSync(
			join(legacyPackageDir, "legacy.ts"),
			"export default (pi) => pi.registerCommand('from-legacy-pi', { description: '', handler() {} });\n",
			"utf8",
		);
