...
describe("PendingSendRegistry", () => {
	test("coalesces identical concurrent sends onto the same promise", async () => {
		...
		assert.equal(
			registry.resolve("message-1", first.attempt.attemptId, {
				id: "message-1",
				delivered: true,
			}),
			true,
	...
	test("rejects reuse of a pending explicit ID for a different logical send", async () => {
		...
		assert.throws(
			() =>
				registry.acquire(
					"message-1",
					buildSendSignature("session-a", { text: "hello", expectsReply: true }),
					1_000,
				),
			/already pending with a different target or payload/,
		);
	...
	test("an old attempt cannot delete or settle a newer owner", async () => {
		...
		assert.equal(
			registry.resolve("message-1", old.attempt.attemptId, {
				id: "message-1",
				delivered: true,
			}),
			false,
		...
		assert.equal(
			registry.resolve("message-1", current.attempt.attemptId, {
				id: "message-1",
				delivered: true,
			}),
			true,
	...
	test("timeout cleanup is attempt-safe when an ID is reused", async () => {
		...
		assert.equal(
			registry.resolveLegacy("message-1", { id: "message-1", delivered: true }),
			false,
			"a late legacy timeout response is ambiguous",
		);
		assert.equal(
			registry.resolve("message-1", expired.attempt.attemptId, {
				id: "message-1",
				delivered: true,
			}),
			false,
	...
	test("disconnect cleanup cannot reject a later replacement", async () => {
		...
		assert.equal(
			registry.resolveLegacy("message-1", { id: "message-1", delivered: false, reason: "late disconnect response" }),
			false,
		);
	...
	test("ignores late legacy delivered and delivery_failed responses after ID replacement", async () => {
		for (const delivered of [true, false]) {
			...
			assert.equal(
				registry.resolveLegacy(replacement.attempt.messageId, {
			...
			assert.equal(
				registry.resolve(replacement.attempt.messageId, replacement.attempt.attemptId, {
					id: replacement.attempt.messageId,
					delivered: true,
				}),
				true,
	...
	test("legacy eligibility expires and generation history remains bounded", async () => {
		...
		assert.equal(
			registry.resolveLegacy("oldest", { id: "oldest", delivered: true }),
			true,
			"evicted history may safely start a new compatibility window",
		);
		...
		assert.equal(
			registry.resolveLegacy("expires", { id: "expires", delivered: false, reason: "legacy failure" }),
			true,
		);
	...
	test("generation retention starts at settlement for long-running original attempts", async () => {
		...
		assert.equal(
			registry.resolve("long-running", replacement.attempt.attemptId, { id: "long-running", delivered: true }),
			true,
		);
		await replacement.attempt.promise;
	});
