...
describe("StageUiBroker", () => {
	...
	describe("headless answering", () => {
		test("answerStagePrompt resolves the pending request, surfaces the descriptor, and notifies listeners", async () => {
			...
			const resolvedAnswer = resolved[0]?.answer as BuiltResult | undefined;
			assert.deepEqual(resolvedAnswer?.answers[0]?.answer, "Blue");
		...
		test("a new prompt clears the previously resolved prompt id", async () => {
			...
			broker.provideStagePrompt(
				"run-1",
				"stage-1",
				buildStagePromptAdapter("old", "ask_user_question", COLOR_ARGS, 1)!,
			);
			...
			broker.provideStagePrompt(
				"run-1",
				"stage-1",
				buildStagePromptAdapter("new", "ask_user_question", COLOR_ARGS, 1)!,
			);
		...
		test("a repeated prompt id clears the prior resolved marker for a fresh gate", async () => {
			...
			broker.provideStagePrompt(
				"run-1",
				"stage-1",
				buildStagePromptAdapter("same", "ask_user_question", COLOR_ARGS, 1)!,
			);
		...
		test("adapter provided after the request still answers and back-fills the descriptor", async () => {
			...
			broker.provideStagePrompt(
				"run-1",
				"stage-1",
				buildStagePromptAdapter("p", "ask_user_question", COLOR_ARGS, 1)!,
			);
