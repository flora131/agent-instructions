...
import type { WorkflowDefinition, WorkflowRuntimeConfig } from "../../packages/workflows/src/shared/types.js";
...
describe("WorkflowRuntimeConfig — DispatcherOpts", () => {
	...
	test("dispatch(list) with config is unaffected", async () => {
		...
		const result = await dispatch({ action: "list" }, { registry, config: sampleConfig });
