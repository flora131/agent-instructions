...
describe("workflow reload rediscovery matrix", () => {
	...
	test.sequential("reload refreshes all discovery scopes and public list/get/inputs/help/completion/invocation surfaces", async () => {
		...
		await Promise.all([
			...
			writeWorkflow(paths.projectLegacy, {
				name: "scope-project-legacy",
			...
			writeWorkflow(paths.projectAbsoluteDir, {
				name: "scope-project-absolute",
				description: "project absolute",
				named: true,
			}),
			writeWorkflow(paths.globalRelative, { name: "scope-global-relative", description: "global relative" }),
			writeWorkflow(paths.globalAbsolute, {
				name: "scope-global-absolute",
				description: "global absolute",
				named: true,
			}),
			writeWorkflow(paths.globalRelativeDir, {
				name: "scope-global-relative-dir",
				description: "global relative directory",
			}),
		...
		for (const expected of [
			"scope-project-atomic",
			"scope-project-legacy",
			"scope-global-atomic",
			"scope-global-legacy",
			"scope-project-relative",
			"scope-project-absolute",
			"scope-global-relative",
			"scope-global-absolute",
			"scope-global-relative-dir",
			"scope-conflict",
		])
	...
	test.sequential("post-start global and configured additions preserve the active registry", async () => {
		...
		assert.deepEqual(
			before.filter((name) => name.startsWith("post-start-")),
			["post-start-existing"],
		);
	...
	test.sequential("add edit rename delete and malformed siblings replace metadata while preserving valid workflows", async () => {
		...
		const stableRun = await harness.execute({
			action: "run",
		...
		assert.match(
			harness.messages.slice(slashMessageStart).join("\n"),
		...
		assert.deepEqual(
			editedInputs.inputs.map((input) => input.name),
			["revisedInput"],
		);
	...
	test.sequential("reload during an in-flight workflow publishes new metadata without changing the running definition", async () => {
		...
		assert.match(
			harness.messages.slice(resumeMessageStart).join("\n"),
