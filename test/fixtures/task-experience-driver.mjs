...
const { values, positionals } = parseArgs({ allowPositionals: true, options: {
 ...
 "fail-after-shell": { type: "boolean", default: false },
} });
...
if (positionals[0] === "wait") {
 ...
 const inspect = async (name) => { tmux("send-keys", "-t", session, "-l", "--", "/tasks"); tmux("send-keys", "-t", session, "Enter"); await rendered("inspect"); capture(name, /t17 Inspect deterministic fixture/); tmux("send-keys", "-t", session, "Escape"); await rendered("inspection-closed"); };
 try {
  tmux("-V");
  const home = join(dirname(directory), `${nonce}-home`); mkdirSync(home);
  // A tmux server can retain another Atomic invocation's engine/session/test markers.
  tmux("new-session", "-d", "-s", session, "-c", cwd, "-x", values.columns, "-y", values.rows, `exec env -i PATH=${quote(process.env.PATH)} HOME=${quote(home)} TERM=xterm-256color bun ${quote(join(cwd, "test/fixtures/task-experience-session.ts"))} --chat ${values.chat} --evidence-dir ${quote(directory)} --run-id ${nonce} 2>${quote(join(dirname(directory), `${nonce}-stderr.log`))}`);
  ...
  const liveRender = await rendered("live", (e) => { assert.equal(e.anchorCount, 0); assert.ok(e.taskIds.includes(started.taskId)); }); assert.equal(liveRender.cursor.generation, started.cursor.generation); assert.ok(BigInt(liveRender.cursor.sequence) >= BigInt(started.cursor.sequence)); capture("live.txt", /Tasks.*1 local agent/);
  await inspect("live-inspector.txt");
  ...
  const persisted = await wait(directory, nonce, "model-persisted", "done", -1, live); assert.equal(persisted.evidence.count, 1); assert.equal(persisted.evidence.display, true); assert.equal(persisted.evidence.completionId, terminal.evidence.completionId);
  const settledRender = await rendered("done", (e) => assert.equal(e.completionCount, 1)); assert.equal(settledRender.cursor.generation, terminal.cursor.generation); assert.ok(BigInt(settledRender.cursor.sequence) >= BigInt(terminal.cursor.sequence)); const settled = capture("settled.txt", /completed/); assert.match(settled, /Fixture complete/); assert.doesNotMatch(settled, /Tasks.*local agent/);
  await inspect("settled-inspector.txt");
  ...
  if (values["fail-after-shell"]) assert.fail("Fixture assertion after shell admission");
