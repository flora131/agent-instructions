...
describe("MockExtensionAPI — slash command registration", () => {
	...
	test("/workflow getArgumentCompletions covers subcommand arguments", async () => {
		...
		assert.equal(
			status.some((c) => c.value === "status --all "),
			false,
		);
		...
		assert.equal(
			quit.some((c) => c.label === "-y" || c.label === "--yes"),
			false,
		);
	});
...
describe("MockExtensionAPI — message renderer registration", () => {
	...
	const REQUIRED_EVENTS = ["workflow.run.start", "workflow.run.end"] as const;
...
describe("renderResult — all action branches", () => {
	...
	test("all compact tool results respect the host render width", () => {
		...
		const cases: WorkflowToolResult[] = [
			...
			},
			{
				...
				runId: "run-abcdef",
				status: "paused",
				message: "A very long resumable quit response message.",
			},
		...
		for (const result of cases) {
			...
			for (const line of out.split("\n")) {
				assert.ok(
					...
					`${result.action} exceeds ${width}: ${visibleWidth(line)} ${JSON.stringify(line)}`,
				);
			}
		}
	});
	...
	test("action='inputs' respects the host render width", () => {
		...
		for (const line of out.split("\n")) {
			assert.ok(
				visibleWidth(line) <= width,
				`inputs exceeds ${width}: ${visibleWidth(line)} ${JSON.stringify(line)}`,
			);
		}
	});
	...
	test("action='inputs' with inputs", () => {
		const out = renderResult({
			action: "inputs",
			name: "wf-y",
			inputs: [{ name: "param1", type: "string", required: true, description: "A param" }],
		});
	...
	test("action='run' background dispatch reuses the slash-command dispatch card", () => {
		...
		for (const line of out.split("\n")) {
			assert.ok(
				visibleWidth(line) <= width,
				`dispatch exceeds ${width}: ${visibleWidth(line)} ${JSON.stringify(line)}`,
			);
		}
	});
