...
- **Draft preservation** - Leaving a stage preserves unsent composer and prompt drafts and keeps pending custom questions unresolved so they reappear when you attach again. An open agent questionnaire retains its selected answers, partial multiselect choices, current tab, inline text, and notes across detach/reattach. These drafts last only for that pending request; cancelling or completing it does not seed a later questionnaire.
