...
**Administrator accounts (Windows).** PostgreSQL refuses to run for a member of the Administrators or Power Users groups, so an Atomic session launched from an elevated terminal or an administrative account would otherwise fail to start the embedded cluster with only a readiness timeout. Atomic starts the retained Postgres process with the same restricted access token `pg_ctl` uses on Windows — the administrative group SIDs become deny-only and every dangerous privilege is removed — while keeping the exact process lease for orderly shutdown. Nothing changes for regular non-administrative Windows accounts. When Postgres exits during startup (a corrupt cluster, a refused setting, or an unrestricted administrator launch), the actual log output is reported instead of a generic "never accepted connections" timeout.
