...
The shared background widget occupies at most **10 rows**, including its scroll hint, and at most one third of the terminal height, rounded down with a one-row minimum. From the main editor, use **Alt+PageUp** and **Alt+PageDown** to page through all rows when the list overflows. The row range at the bottom shows your position. Ordinary typing, arrows, PageUp/PageDown, and mouse scrolling keep their existing editor and transcript behavior.
