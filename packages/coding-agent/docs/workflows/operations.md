...
- **Pending questions during control** - Pausing or quitting an agent stage cancels its open questionnaire without treating it as an answer. Resume the workflow to continue; the agent can ask again. A resumed live workflow can run subsequent workflow tools and save its results.
