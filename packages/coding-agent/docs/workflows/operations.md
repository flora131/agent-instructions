...
Attached prompt questions display terminal-control bytes as literal `\xNN` text instead of executing them. Completed prompt archives apply the same protection to questions, choices, initial values, and responses. Line breaks and readable Unicode remain visible; tabs expand to spaces. This display protection does not change the stored question or your submitted response.
