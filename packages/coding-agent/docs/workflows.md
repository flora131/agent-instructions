...
Design for elapsed time using [runtime-aware scheduling and estimates](/workflows/reliable-design#runtime-aware-scheduling-and-estimates). Use available CI/test history to place focused checks at slice boundaries, defer redundant full-suite runs to the final candidate, and overlap independent work with CI without bypassing required gates. Before launch, report a completion range without confidence labels or scores, then use [`ask_user_question`](/tools#ask_user_question) or an equivalent usable question tool to offer proceeding with inherited limits, setting an explicit budget, or not launching. Wait for the answer, unless the user already supplied a budget choice. If no usable question tool exists, continue autonomously using best judgment while preserving existing limits and approval gates. An estimate is not a budget. At completion, compare the estimate with actual elapsed time.
