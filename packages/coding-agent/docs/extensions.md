...
An executing descendant with an explicit `rootRunId` retains its root's `stopping` reason even when the root snapshot is absent. Removing history does not release runtime execution or stop ownership.
