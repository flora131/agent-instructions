...
Each reporter instance binds to the first eligible session's `SessionManager`, before awaiting pane acquisition. If a supplied resource loader shares that instance with another runner, the other session's start, activity, prompt, and shutdown events are ignored. In particular, a child quitting cannot release the parent's pane or cancel its pending claim. Normal compaction retains the same session and reporter; subsequent work continues reporting without reclaiming authority. Owning-session reload and replacement still drain and release in serialized order before the successor reports, and genuine quit leaves release as the final command.
