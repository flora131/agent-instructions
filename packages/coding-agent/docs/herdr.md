...
Each reporter instance binds to an eligible session's `SessionManager` before awaiting pane acquisition. While that session owns the reporter, other sessions sharing the loaded instance cannot change its activity, prompts, or lifecycle. In particular, a child quitting cannot release the parent's pane or cancel its pending claim. The owning session's shutdown clears this binding before awaiting transport cleanup, allowing a subsequent eligible session to reuse the same already-loaded resource loader. The successor reserves its binding immediately but waits for predecessor retirement before reporting its own identity; neither delayed predecessor events nor completion of the old shutdown can clear the new binding.

Normal compaction retains the same session and reporter; subsequent work continues reporting without reclaiming authority. Owning-session reload and replacement drain and release in serialized order before successor reports. Genuine quit releases the pane, and late activity or prompt events from the quitting session do not reclaim it.
