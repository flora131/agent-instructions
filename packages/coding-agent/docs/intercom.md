...
Working subagents accept `send` and `ask` as steering, just like live workflow stages: the message joins the same task and is available at the next model turn. Active tools finish normally; Intercom does not cancel the child or launch another task. This works with foreground and background subagents. Messages received during startup join the original task, and multiple messages retain arrival order. Use an exact connected child name or full session ID from `intercom list`; subagents are not workflow-stage paths.
