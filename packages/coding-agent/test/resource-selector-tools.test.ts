...
function patchZipMethod(file: string, entryName: string, method: number): void {
	...
	for (let n = 0; n < buf.readUInt16LE(eocd + 10); n++) {
		...
		if (name === entryName) {
			buf.writeUInt16LE(method, ptr + 10);
			buf.writeUInt16LE(method, localOffset + 8);
			break;
		}
...
describe("resource selector tools", () => {
	...
	it("reads writes and searches zip archive members", async () => {
		...
		await write.execute(
			"zip-write",
			{ path: "bundle.zip:src/a.txt", content: "alpha\nneedle\nabc\n" },
			undefined,
			undefined,
			{} as never,
		);
		const readOutput = textOutput(
			await createReadToolDefinition(testDir).execute(
				"zip-read",
				{ path: "bundle.zip:src/a.txt" },
				undefined,
				undefined,
				{} as never,
			),
		...
		const searchOutput = textOutput(
			await createSearchToolDefinition(testDir).execute(
				"zip-search",
				{ pattern: "needle", paths: "bundle.zip:src/a.txt" },
				undefined,
				undefined,
				{} as never,
			),
		...
		const regexOutput = textOutput(
			await createSearchToolDefinition(testDir).execute(
				"zip-regex",
				{ pattern: "a.c", paths: "bundle.zip:src/a.txt" },
				undefined,
				undefined,
				{} as never,
			),
	...
	it("keeps archive regex semantics and splits delimiter-joined archive search paths", async () => {
		...
		await write.execute(
			"zip-case-write",
			{ path: "case.zip:a.txt", content: "Needle\n" },
			undefined,
			undefined,
			{} as never,
		);
		await write.execute(
			"zip-delim-a",
			{ path: "d.zip:a.txt", content: "needle a\n" },
			undefined,
			undefined,
			{} as never,
		);
		await write.execute(
			"zip-delim-b",
			{ path: "d.zip:b.txt", content: "needle b\n" },
			undefined,
			undefined,
			{} as never,
		);
		const inline = textOutput(
			await createSearchToolDefinition(testDir).execute(
				"zip-inline-case",
				{ pattern: "(?i)needle", paths: "case.zip:a.txt" },
				undefined,
				undefined,
				{} as never,
			),
		...
		const split = textOutput(
			await createSearchToolDefinition(testDir).execute(
				"zip-delim-search",
				{ pattern: "needle", paths: "d.zip:a.txt d.zip:b.txt" },
				undefined,
				undefined,
				{} as never,
			),
	...
	it("honors verbose regex comments for archive and internal resource searches", async () => {
		await createWriteToolDefinition(testDir).execute(
			"zip-verbose-comment",
			{ path: "verbose.zip:a.txt", content: "foobar\n" },
			undefined,
			undefined,
			{} as never,
		);
		const pattern = "(?x)foo # comment\n bar";
		const archiveOutput = textOutput(
			await createSearchToolDefinition(testDir).execute(
				"zip-verbose-comment-search",
				{ pattern, paths: "verbose.zip:a.txt" },
				undefined,
				undefined,
				{} as never,
			),
		...
		const internalOutput = textOutput(
			await createSearchToolDefinition(testDir).execute(
				"internal-verbose-comment-search",
				{ pattern, paths: "memory://note" },
				undefined,
				undefined,
				ctx,
			),
	...
	it("preserves archive members with delimiters and ripgrep inline flags", async () => {
		...
		await write.execute(
			"zip-space-member",
			{ path: "space.zip:a b.txt", content: "needle spaced\n" },
			undefined,
			undefined,
			{} as never,
		);
		writeFileSync(join(testDir, "b.txt"), "needle wrong\n");
		const spaced = textOutput(
			await createSearchToolDefinition(testDir).execute(
				"zip-space-search",
				{ pattern: "needle", paths: "space.zip:a b.txt" },
				undefined,
				undefined,
				{} as never,
			),
		...
		await write.execute(
			"zip-multiline",
			{ path: "m.zip:a.txt", content: "foo\nbar\n" },
			undefined,
			undefined,
			{} as never,
		);
		const multiline = textOutput(
			await createSearchToolDefinition(testDir).execute(
				"zip-inline-m",
				{ pattern: "(?m)^bar", paths: "m.zip:a.txt" },
				undefined,
				undefined,
				{} as never,
			),
		...
		await write.execute(
			"zip-extended",
			{ path: "x.zip:a.txt", content: "needle\n" },
			undefined,
			undefined,
			{} as never,
		);
		const extended = textOutput(
			await createSearchToolDefinition(testDir).execute(
				"zip-inline-x",
				{ pattern: "(?x)n e e d l e", paths: "x.zip:a.txt" },
				undefined,
				undefined,
				{} as never,
			),
	...
	it("does not steal archive member names that look like read suffixes", async () => {
		...
		const rawRange = textOutput(
			await createReadToolDefinition(testDir).execute(
				"read-zip-raw-range",
				{ path: "z.zip:raw:2-2:raw" },
				undefined,
				undefined,
				{} as never,
			),
	...
	it("honors raw and conflict suffixes for extensionless archive members", async () => {
		...
		await write.execute(
			"zip-readme",
			{ path: "extless.zip:README", content: "hello\n" },
			undefined,
			undefined,
			{} as never,
		);
		...
		const output = textOutput(
			await createReadToolDefinition(testDir).execute(
				"read-extless-conflict",
				{ path: "extless.zip:CONFLICT:conflicts" },
				undefined,
				undefined,
				{} as never,
			),
	...
	it("honors archive read suffixes for colon members and before ranges", async () => {
		...
		await write.execute(
			"zip-colon-member",
			{ path: "colon.zip:dir:file.txt", content: "needle\n" },
			undefined,
			undefined,
			{} as never,
		);
		expect(
			textOutput(
				await createReadToolDefinition(testDir).execute(
					"read-colon-raw",
					{ path: "colon.zip:dir:file.txt:raw" },
					undefined,
					undefined,
					{} as never,
				),
		...
		await write.execute(
			"zip-literal-colon-raw",
			{ path: "colon.zip:dir:file.txt:raw", content: "literal\n" },
			undefined,
			undefined,
			{} as never,
		);
		...
		await write.execute(
			"zip-range-conflict",
			{ path: "combo.zip:file", content: "pre\n<<<<<<< ours\nleft\n=======\nright\n>>>>>>> theirs\npost\n" },
			undefined,
			undefined,
			{} as never,
		);
		const output = textOutput(
			await createReadToolDefinition(testDir).execute(
				"read-conflict-before-range",
				{ path: "combo.zip:file:conflicts:1-5" },
				undefined,
				undefined,
				{} as never,
			),
	...
	it("filters conflict-only reads for archive members", async () => {
		...
		const output = textOutput(
			await createReadToolDefinition(testDir).execute(
				"read-zip-conflicts",
				{ path: "conflict.zip:file.txt:conflicts" },
				undefined,
				undefined,
				{} as never,
			),
	...
	it("reads and searches archive members named like line selectors", async () => {
		...
		await write.execute(
			"zip-l-number-member",
			{ path: "num.zip:L1", content: "letter\n" },
			undefined,
			undefined,
			{} as never,
		);
		...
		expect(
			textOutput(
				await createSearchToolDefinition(testDir).execute(
					"search-zip-number-member",
					{ pattern: "needle", paths: "num.zip:1" },
					undefined,
					undefined,
					{} as never,
				),
	...
	it("does not strip suffix-looking archive member path segments", async () => {
		await createWriteToolDefinition(testDir).execute(
			"zip-raw-segment",
			{ path: "seg.zip:raw:notes.txt", content: "right\n" },
			undefined,
			undefined,
			{} as never,
		);
		await createWriteToolDefinition(testDir).execute(
			"zip-plain-notes",
			{ path: "seg.zip:notes.txt", content: "wrong\n" },
			undefined,
			undefined,
			{} as never,
		);
		...
		await createWriteToolDefinition(testDir).execute(
			"zip-conflicts-segment",
			{ path: "seg.zip:conflicts:notes.txt", content: "conflict name\n" },
			undefined,
			undefined,
			{} as never,
		);
		expect(
			textOutput(
				await createReadToolDefinition(testDir).execute(
					"read-conflicts-segment",
					{ path: "seg.zip:conflicts:notes.txt" },
					undefined,
					undefined,
					{} as never,
				),
			),
		).toContain("conflict name");
	});

	it("truncates oversized URL reads through the fetch pipeline", async () => {
		const server = createServer((_req, res) => {
			res.setHeader("content-type", "text/plain");
			res.end("x".repeat(60_000));
		});
		const port = await listen(server);
		try {
			const result = await createReadToolDefinition(testDir).execute(
				"url-oversized",
				{ path: `http://127.0.0.1:${port}/large.txt` },
				undefined,
				undefined,
				{} as never,
			);
	...
	it("surfaces pagination for archive search pages", async () => {
		...
		for (let index = 1; index <= 21; index++)
			await write.execute(
				"zip-page-write",
				{ path: `page.zip:f${String(index).padStart(2, "0")}.txt`, content: "needle\n" },
				undefined,
				undefined,
				{} as never,
			);
		...
		const next = textOutput(
			await createSearchToolDefinition(testDir).execute(
				"zip-page-search-skip",
				{ pattern: "needle", paths: "page.zip:", skip: 20 },
				undefined,
				undefined,
				{} as never,
			),
	...
	it("resolves archive paths when probing later search pages", async () => {
		for (let index = 1; index <= 20; index++)
			writeFileSync(join(testDir, `first-${String(index).padStart(2, "0")}.txt`), "needle\n");
		await createWriteToolDefinition(testDir).execute(
			"later-zip-write",
			{ path: "later.zip:a.txt", content: "needle later\n" },
			undefined,
			undefined,
			{} as never,
		);
		const paths = [
			...Array.from({ length: 20 }, (_, index) => `first-${String(index + 1).padStart(2, "0")}.txt`),
			"later.zip:",
		];
		const output = textOutput(
			await createSearchToolDefinition(testDir).execute(
	...
	it("does not return resource search context when selected range has no match", async () => {
		await createWriteToolDefinition(testDir).execute(
			"zip-range-write",
			{ path: "range.zip:a.txt", content: "alpha\nneedle\nomega\n" },
			undefined,
			undefined,
			{} as never,
		);
		const output = textOutput(
			await createSearchToolDefinition(testDir).execute(
				"zip-range-miss",
				{ pattern: "needle", paths: "range.zip:a.txt:1-1" },
				undefined,
				undefined,
				{} as never,
			),
	...
	it("writes selected zip members without inflating unrelated entries", async () => {
		...
		await write.execute(
			"zip-write-small",
			{ path: "mixed.zip:small.txt", content: "old\n" },
			undefined,
			undefined,
			{} as never,
		);
		await write.execute(
			"zip-write-bad",
			{ path: "mixed.zip:bad.bin", content: "bad\n" },
			undefined,
			undefined,
			{} as never,
		);
		...
		await write.execute(
			"zip-rewrite-small",
			{ path: "mixed.zip:small.txt", content: "new\n" },
			undefined,
			undefined,
			{} as never,
		);
	...
	it("blocks oversized archive member reads like file reads", async () => {
		await createWriteToolDefinition(testDir).execute(
			"zip-write-big",
			{ path: "big.zip:big.txt", content: `sentinel\n${"x".repeat(50_001)}` },
			undefined,
			undefined,
			{} as never,
		);
	...
	it("blocks multibyte archive member reads by byte size", async () => {
		await createWriteToolDefinition(testDir).execute(
			"zip-write-emoji",
			{ path: "emoji.zip:big.txt", content: "🙂".repeat(20_000) },
			undefined,
			undefined,
			{} as never,
		);
	...
	it("pages search results across archive and filesystem targets", async () => {
		await createWriteToolDefinition(testDir).execute(
			"zip-write-skip",
			{ path: "bundle.zip:a.txt", content: "needle zip\n" },
			undefined,
			undefined,
			{} as never,
		);
		...
		const output = textOutput(
			await createSearchToolDefinition(testDir).execute(
				"search-skip-resource",
				{ pattern: "needle", paths: ["bundle.zip:a.txt", "b.txt"], skip: 1 },
				undefined,
				undefined,
				{} as never,
			),
	...
	it("limits first-page archive search results", async () => {
		...
		for (let index = 1; index <= 21; index++) {
			await write.execute(
				`zip-write-page-${index}`,
				{ path: `many.zip:${String(index).padStart(2, "0")}.txt`, content: `needle ${index}\n` },
				undefined,
				undefined,
				{} as never,
			);
		...
		const output = textOutput(
			await createSearchToolDefinition(testDir).execute(
				"search-resource-page-limit",
				{ pattern: "needle", paths: "many.zip:" },
				undefined,
				undefined,
				{} as never,
			),
	...
	it("searches multiline regex patterns in archive and internal resources", async () => {
		await createWriteToolDefinition(testDir).execute(
			"zip-write-multiline",
			{ path: "multi.zip:a.txt", content: "foo\nbar\n" },
			undefined,
			undefined,
			{} as never,
		);
		const archiveOutput = textOutput(
			await createSearchToolDefinition(testDir).execute(
				"search-archive-multiline",
				{ pattern: "foo\\nbar", paths: "multi.zip:a.txt" },
				undefined,
				undefined,
				{} as never,
			),
		...
		const internalOutput = textOutput(
			await createSearchToolDefinition(testDir).execute(
				"search-internal-multiline",
				{ pattern: "foo\\nbar", paths: "memory://multi" },
				undefined,
				undefined,
				ctx as never,
			),
	...
	it("reads writes and searches tar archive members without host python", async () => {
		...
		try {
			...
			await write.execute(
				"tar-write",
				{ path: "bundle.tar:src/a.txt", content: "alpha\nneedle\n" },
				undefined,
				undefined,
				{} as never,
			);
			...
			const searchOutput = textOutput(
				await createSearchToolDefinition(testDir).execute(
					"tar-search",
					{ pattern: "needle", paths: "bundle.tar:src/a.txt" },
					undefined,
					undefined,
					{} as never,
				),
	...
	it("treats existing non-SQLite .db files as plain files", async () => {
		...
		expect(
			textOutput(
				await createReadToolDefinition(testDir).execute(
					"sqlite-conflicts-table",
					{ path: "tokens.sqlite:conflicts" },
					undefined,
					undefined,
					{} as never,
				),
			),
		).toContain("Conflict Table");
		expect(
			textOutput(
				await createSearchToolDefinition(testDir).execute(
					"plain-db-search-line",
					{ pattern: "two", paths: "notes.db:2-2" },
					undefined,
					undefined,
					{} as never,
				),
	...
	it("creates missing bare .db paths as plain files", async () => {
		await createWriteToolDefinition(testDir).execute(
			"plain-db-create",
			{ path: "new-notes.db", content: "hello\n" },
			undefined,
			undefined,
			{} as never,
		);
	...
	it("reads writes and searches SQLite selectors", async () => {
		...
		await createWriteToolDefinition(testDir).execute(
			"sqlite-write",
			{ path: "data.sqlite:users:42", content: "{name: 'Ada',}" },
			undefined,
			undefined,
			{} as never,
		);
		await createWriteToolDefinition(testDir).execute(
			"sqlite-insert",
			{ path: "data.sqlite:users", content: "{id: '7', name: 'Grace'}" },
			undefined,
			undefined,
			{} as never,
		);
		await createWriteToolDefinition(testDir).execute(
			"sqlite-json5-numbers",
			{ path: "data.sqlite:numbers", content: "{flags: 0x10, score: +1}" },
			undefined,
			undefined,
			{} as never,
		);
		expect(
			textOutput(
				await createReadToolDefinition(testDir).execute(
					"sqlite-json5-numbers-read",
					{ path: "data.sqlite:numbers:1" },
					undefined,
					undefined,
					{} as never,
				),
		...
		await createWriteToolDefinition(testDir).execute(
			"sqlite-hex-string",
			{ path: "data.sqlite:users:42", content: "{name: '0x10'}" },
			undefined,
			undefined,
			{} as never,
		);
		expect(
			textOutput(
				await createReadToolDefinition(testDir).execute(
					"sqlite-hex-string-read",
					{ path: "data.sqlite:users:42" },
					undefined,
					undefined,
					{} as never,
				),
		...
		expect(
			textOutput(
				await createReadToolDefinition(testDir).execute(
					"sqlite-big-read",
					{ path: "big.sqlite:blobs" },
					undefined,
					undefined,
					{} as never,
				),
			),
		).toContain("File read blocked");
		await createWriteToolDefinition(testDir).execute(
			"sqlite-json5-comments",
			{ path: "data.sqlite:users:42", content: "{name: 'Ada', // user\n}" },
			undefined,
			undefined,
			{} as never,
		);
		const rawTable = textOutput(
			await createReadToolDefinition(testDir).execute(
				"sqlite-raw-table",
				{ path: "data.sqlite:users:raw" },
				undefined,
				undefined,
				{} as never,
			),
		...
		const rangedListing = textOutput(
			await createReadToolDefinition(testDir).execute(
				"sqlite-range-listing",
				{ path: "data.sqlite:1-1" },
				undefined,
				undefined,
				{} as never,
			),
		...
		const rangedOob = textOutput(
			await createReadToolDefinition(testDir).execute(
				"sqlite-range-oob",
				{ path: "data.sqlite:99-99" },
				undefined,
				undefined,
				{} as never,
			),
		...
		const readRawRow = textOutput(
			await createReadToolDefinition(testDir).execute(
				"sqlite-read-raw-row",
				{ path: "data.sqlite:users:42:raw" },
				undefined,
				undefined,
				{} as never,
			),
		...
		const searchOutput = textOutput(
			await createSearchToolDefinition(testDir).execute(
				"sqlite-search",
				{ pattern: "Ada", paths: "data.sqlite:users" },
				undefined,
				undefined,
				{} as never,
			),
		...
		const regexOutput = textOutput(
			await createSearchToolDefinition(testDir).execute(
				"sqlite-regex",
				{ pattern: "A.a", paths: "data.sqlite:users" },
				undefined,
				undefined,
				{} as never,
			),
		...
		const insensitiveOutput = textOutput(
			await createSearchToolDefinition(testDir).execute(
				"sqlite-insensitive",
				{ pattern: "ada", paths: "data.sqlite:users", i: true },
				undefined,
				undefined,
				{} as never,
			),
		...
		const inlineSqlite = textOutput(
			await createSearchToolDefinition(testDir).execute(
				"sqlite-inline-i",
				{ pattern: "(?i)ada", paths: "data.sqlite:users" },
				undefined,
				undefined,
				{} as never,
			),
		...
		const listingSearch = textOutput(
			await createSearchToolDefinition(testDir).execute(
				"sqlite-listing-range-search",
				{ pattern: "users", paths: "data.sqlite:2-2" },
				undefined,
				undefined,
				{} as never,
			),
		...
		const rowSearch = textOutput(
			await createSearchToolDefinition(testDir).execute(
				"sqlite-row-search",
				{ pattern: "Ada", paths: "data.sqlite:users:42" },
				undefined,
				undefined,
				{} as never,
			),
		...
		const textKeySearch = textOutput(
			await createSearchToolDefinition(testDir).execute(
				"sqlite-text-key-search",
				{ pattern: "Ada Text", paths: "data.sqlite:users" },
				undefined,
				undefined,
				{} as never,
			),
		...
		implicitDb.close();
		const implicitSearch = textOutput(
			await createSearchToolDefinition(testDir).execute(
				"sqlite-implicit-rowid-search",
				{ pattern: "needle", paths: "implicit.sqlite:notes" },
				undefined,
				undefined,
				{} as never,
			),
		...
		expect(
			textOutput(
				await createReadToolDefinition(testDir).execute(
					"sqlite-implicit-rowid-read",
					{ path: "implicit.sqlite:notes:2" },
					undefined,
					undefined,
					{} as never,
				),
			),
		).toContain("needle row");
		const paged = textOutput(
			await createReadToolDefinition(testDir).execute(
				"sqlite-page",
				{ path: "data.sqlite:users?limit=1&offset=2&order=name:asc" },
				undefined,
				undefined,
				{} as never,
			),
		...
		const filtered = textOutput(
			await createReadToolDefinition(testDir).execute(
				"sqlite-filter",
				{ path: "data.sqlite:users?where=name%3D'Ada'" },
				undefined,
				undefined,
				{} as never,
			),
		...
		await expect(
			createReadToolDefinition(testDir).execute(
				"sqlite-bad-filter",
				{ path: "data.sqlite:users?where=1%3D1%20LIMIT%201000000%20--&limit=1" },
				undefined,
				undefined,
				{} as never,
			),
		).rejects.toThrow(/Invalid SQLite where/);
		...
		manyDb.close();
		const sample = textOutput(
			await createReadToolDefinition(testDir).execute(
				"sqlite-sample",
				{ path: "many.sqlite:rows" },
				undefined,
				undefined,
				{} as never,
			),
		...
		await createWriteToolDefinition(testDir).execute(
			"sqlite-delete",
			{ path: "data.sqlite:users:7", content: "" },
			undefined,
			undefined,
			{} as never,
		);
		...
		await expect(
			createWriteToolDefinition(testDir).execute(
				"sqlite-missing",
				{ path: "missing.sqlite:users:1", content: JSON.stringify({ name: "Nope" }) },
				undefined,
				undefined,
				{} as never,
			),
		...
		await expect(
			createWriteToolDefinition(testDir).execute(
				"sqlite-empty-table",
				{ path: "data.sqlite:users", content: "" },
				undefined,
				undefined,
				{} as never,
			),
		...
		await expect(
			createWriteToolDefinition(testDir).execute(
				"sqlite-unknown-query-write",
				{ path: "data.sqlite:users?foo=bar", content: "{name:'Ada'}" },
				undefined,
				undefined,
				{} as never,
			),
		...
		await createWriteToolDefinition(testDir).execute(
			"sqlite-long",
			{ path: "data.sqlite:users:42", content: `{name:'${"x".repeat(2200)}needle'}` },
			undefined,
			undefined,
			{} as never,
		);
		const longSearch = textOutput(
			await createSearchToolDefinition(testDir).execute(
				"sqlite-long-search",
				{ pattern: "needle", paths: "data.sqlite:users:42" },
				undefined,
				undefined,
				{} as never,
			),
	...
	it("reads searches and writes internal artifact resources through the session router", async () => {
		...
		const ctx: InternalResourceContext = {
			internalRouter: {
				read: (url) => resources.get(url) ?? "",
				write: (url, content) => {
		...
		await createWriteToolDefinition(testDir).execute(
			"artifact-write",
			{ path: "artifact://notes/a.txt", content: "needle\n" },
			undefined,
			undefined,
			ctx as never,
		);
		await expect(
			createWriteToolDefinition(testDir).execute(
				"conflict-scope-write",
				{ path: "conflict://abc/ours", content: "resolved\n" },
				undefined,
				undefined,
				{} as never,
			),
		...
		expect(
			textOutput(
				await createSearchToolDefinition(testDir).execute(
					"artifact-search",
					{ paths: "artifact://notes/a.txt", pattern: "n.edle" },
					undefined,
					undefined,
					ctx as never,
				),
	...
	it("splices conflict id and bulk writes without clobbering surrounding content", async () => {
		...
		await createWriteToolDefinition(testDir).execute(
			"conflict-one",
			{ path: "conflict://1", content: "@theirs\n" },
			undefined,
			undefined,
			{} as never,
		);
		...
		await createWriteToolDefinition(testDir).execute(
			"conflict-all",
			{ path: "conflict://*", content: "resolved\n" },
			undefined,
			undefined,
			{} as never,
		);
	...
	it("rejects generated plain-file overwrites", async () => {
		...
		await expect(
			createWriteToolDefinition(testDir).execute(
				"generated-write",
				{ path: "gen.ts", content: "new\n" },
				undefined,
				undefined,
				{} as never,
			),
		).rejects.toThrow(/generated file/);
	});
	...
	it("truncates resource-backed search lines", async () => {
		...
		const output = textOutput(
			await createSearchToolDefinition(testDir).execute(
				"resource-long-line",
				{ paths: "memory://note", pattern: "needle" },
				undefined,
				undefined,
				ctx as never,
			),
	...
	it("converts notebooks to editable cells", async () => {
		writeFileSync(
			join(testDir, "big.ipynb"),
			JSON.stringify({ cells: [{ cell_type: "code", source: ["x".repeat(60_000)] }] }),
		);
		expect(
			textOutput(
				await createReadToolDefinition(testDir).execute(
		...
		const outOfRange = textOutput(
			await createReadToolDefinition(testDir).execute(
				"ipynb-oob",
				{ path: "notebook.ipynb:99" },
				undefined,
				undefined,
				{} as never,
			),
