...
In `auto` mode (default), `web_search` tries Exa first (direct API if keyed, MCP if not), then Perplexity, then You.com, then Gemini API, then Gemini Web when browser-cookie access is enabled.
...
Search the web via Exa, Perplexity AI, You.com, or Gemini. Returns a synthesized answer with source citations. The curator UI is off by default; enable it with `/curator on` or `"workflow": "summary-review"` in config.
...
| `provider` | `auto` (default), `exa`, `perplexity`, `youcom`, or `gemini` |
...
| `youcom.ts` | You.com Search API client (web + news results) |
