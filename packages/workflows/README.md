...
The session starts at `user-feedback-N-start`; durable `live-poll-N-M` and `live-reply-N-M` tool nodes own polling and acknowledgements, while `live-generate-*`, `live-steer-*`, `live-manual_edit_apply-*`, and `live-variant_mount_failed-*` mint model stages. Replies pass the event id before the status. Successful `variant_mounted` events are journal-only; `accept`, `discard`, and `prefetch` mint no model stage, and `timeout` is absorbed in the poll node. Nonzero helper exits fail the run rather than looking like timeouts. No `<artifact_dir>/feedback/` directory or per-session record is written.

Impeccable 4.1.1 persists the selected app root for all Live helpers and supports SvelteKit, Nuxt, TanStack Start, Astro, Next.js, Vite, and static HTML injection. Configured files and generated adapter paths must remain project-relative, inside the real app root, and outside symlinked parents; the browser opener accepts only loopback HTTP(S) review URLs.
