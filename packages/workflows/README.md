...
```json
{
  ...
  "parameters": {
    ...
    "runId": "optional full 36-character run UUID or unique 8-character hexadecimal UUID prefix; other truncations and ambiguous prefixes are rejected; control actions default to the active run where safe; use '--all' or all:true for pause/quit all",
    "stageId": "optional exact stage id/name or unique 8-hex prefix of a bare stage UUID; partial names, composite-ID prefixes, and tool-node prefixes are rejected; cannot be combined with all:true",
