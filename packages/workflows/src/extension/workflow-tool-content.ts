...
function statusAwaitingInputLine(entry: WorkflowStatusAwaitingInput): string {
	const target =
		entry.stageId !== undefined ? `stage ${entry.stageName ?? entry.stageId} (${entry.stageId})` : "run-level prompt";
...
function renderTranscriptToolContent(result: Extract<WorkflowToolResult, { action: "transcript" }>): string {
	const lines = [
		"action: transcript",
		`runId: ${result.runId}`,
...
function renderStagesToolContent(result: Extract<WorkflowToolResult, { action: "stages" }>): string {
...
function renderStageToolContent(result: Extract<WorkflowToolResult, { action: "stage" }>): string {
...
export function workflowGetResult(runtime: ExtensionRuntime, args: WorkflowToolArgs): WorkflowToolResult {
