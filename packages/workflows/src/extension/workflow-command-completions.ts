...
function completeToken(argumentText: string, candidates: PiArgumentCompletion[]): PiArgumentCompletionResult {
...
export function workflowArgumentCompletions(partial: string, runtime: ExtensionRuntime): PiArgumentCompletionResult {
	...
	const inputCompletions: PiArgumentCompletion[] = Object.entries(workflow.inputs).map(([name, schema]) => ({
		value: `${name}=`,
		label: name,
