...
function targetStage(store: Store, target: ResumeAcknowledgementTarget): StageSnapshot | undefined {
	return store
		.runs()
...
export async function settleResumeAcknowledgements(
	...
	settled.forEach((result, index) => {
		...
		const controlVisiblyRunning =
			target.handle.status === "running" ||
...
export function resumeAcknowledgementMessage(
	...
	return `Resume acknowledged; workflow ${runId} ${
		current.endedAt !== undefined ? `reached terminal status ${current.status}` : "can continue"
	}.`;
}
