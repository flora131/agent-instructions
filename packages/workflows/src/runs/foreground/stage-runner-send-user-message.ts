...
function coordinatedPromptOwnershipObserver(turn: StageMessageTurn): PromptOwnershipObserver {
	return {
		arm: () => turn.arm(),
...
function unsupportedContentError(): Error {
	return new Error(
		"atomic-workflows: this stage session adapter does not support non-string sendUserMessage content; provide a runtime sendUserMessage implementation for text/image blocks.",
	);
}
